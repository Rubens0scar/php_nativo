...
{var ficha=document.getElementById(imprime);var ventimp=window.open(' ','popimpr');ventimp.document.write(ficha.innerHTML);ventimp.document.close();ventimp.print();ventimp.close();}
    ...
    <center>
	  <a href="javascript:imprSelec('imprime')"><img src="../theme/images/print.png" width="30" height="30"/ align="right"></a>
		...
		<table><tr><td class="titulo">REPORTE EXTENDIDO DE FECHAS Y GRUPO CONTABLE</td></tr>            
            <tr><td class="subtitulo" style="text-align: center">Reporte Restaurante "SABOR GAUCHO"</td></tr>
        </table><br>
...
$db = Core::Conectar();
 ...
 $consulta ="SELECT gam.cod_resumen, ri.id_activo, ri.id_registro_individual, year(ra.fecha_compra) as anio, ra.fecha_compra, ra.n_cbt, ac.nombre, 
                ra.n_adjuntos, ri.descripcion_act, ra.factura, ri.costo, e.est_tec, ri.observaciones, p.nombres, p.apaterno, p.amaterno, 
                c.descripcion as cargo, p.ubicacion,  
                case ra.estado when 1 then 'ACTIVO' else 'INACTIVO' end estado, tc.ufv_venta 
