...
if (!extension_loaded('mbstring')) {
  ...
  if (!function_exists('mb_convert_encoding')) {
    function mb_convert_encoding($data, $to_encoding, $from_encoding = 'UTF-8') {
        // Verificamos si se está solicitando codificación de entidades HTML
        if (str_replace('-', '', strtolower($to_encoding)) === 'htmlentities') {
            return htmlentities($data, ENT_QUOTES, $from_encoding);
        } elseif (str_replace('-', '', strtolower($to_encoding)) === 'utf8') {
            return utf8_encode($data);
        }

        // Para otros casos, se devuelve el texto con decodificación UTF-8
        return utf8_decode($data);
    }
}
